# 🏎️ GPU Scheduler & K8s Allocator Studio
...
[![Studio](https://img.shields.io/badge/Developer_Studio-Live-brightgreen)](https://pradeeptalari14.github.io/portfolio/tools/gpu-scheduler/)
...
| Tab | Description |
|-----|-------------|
| `gpu-policy.yaml` | Generated in studio Output tab |
| `kueue-config.yaml` | Generated in studio Output tab |
| `Flow Diagram` | Generated in studio Output tab |

### Configurable Options
| Option | Available Values |
|--------|-----------------|
| **GPU Sharing Mode** | `NVIDIA MIG Partitioning` / `MPS / Fractional Shared GPUs` / `Direct Passthrough` |
| **Queue Scheduling Policy** | `Weighted Fair Share` / `Strict Priority Preemption` / `First-In First-Out` |
| **Allocated GPU Limit Weight** | `1` / `2` / `4` / `8` / `16` |
...
```mermaid
graph TD
  Job[🚀 Batch Training Job] -->|Submit| Queue[⚡ Kueue ClusterQueue]
  Queue -->|Evaluate resources| Alloc[🎛️ Allocator strategy: spread]
  Alloc -->|Apply partition| Share[🏎️ GPU Share]
  Share -->|Reserve GPU units| GPU[🤖 Physical NVIDIA GPU Cluster]
```
...
```bash
git clone https://github.com/Pradeeptalari14/tp-gpu-scheduler.git
cd tp-gpu-scheduler
```
...
```bash
kubectl port-forward svc/tp-gpu-scheduler 8501:8501
```
...
# Primary Config File: gpu-policy.yaml
```
...
### GPU Sharing Mode
- **`NVIDIA MIG Partitioning`** — see `examples/` folder for generated output
- **`MPS / Fractional Shared GPUs`** — see `examples/` folder for generated output
- **`Direct Passthrough`** — see `examples/` folder for generated output

### Queue Scheduling Policy
- **`Weighted Fair Share`** — see `examples/` folder for generated output
- **`Strict Priority Preemption`** — see `examples/` folder for generated output
- **`First-In First-Out`** — see `examples/` folder for generated output

### Allocated GPU Limit Weight
- **`1`** — see `examples/` folder for generated output
- **`2`** — see `examples/` folder for generated output
- **`4`** — see `examples/` folder for generated output
...
| Resource | Link |
|----------|------|
| Interactive Studio | [Open →](https://pradeeptalari14.github.io/portfolio/tools/gpu-scheduler/) |
...
---
*Generated by [GPU Scheduler & K8s Allocator Studio Studio](https://pradeeptalari14.github.io/portfolio/tools/gpu-scheduler/) — [Talari Pradeep Portfolio](https://pradeeptalari14.github.io/portfolio)*